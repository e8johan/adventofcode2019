            ...
            if s1[4] == 0:
                ...
                if s2[4] == 0:
                    ...
                    if s1[0] > min(s2[0], s2[0] + s2[4]) and s1[0] < max(s2[0], s2[0] + s2[4]) and s2[1] > min(s1[1], s1[1] + s1[5]) and s2[1] < max(s1[1], s1[1] + s1[5]):
                        res.append(d1 + abs(s1[1]-s2[1]) + d2 + abs(s1[0]-s2[0]))
            else: # s1[5] == 0
                ...
                if s2[4] == 0:
                    ...
                    if s1[1] > min(s2[1], s2[1] + s2[5]) and s1[1] < max(s2[1], s2[1] + s2[5]) and s2[0] > min(s1[0], s1[0] + s1[4]) and s2[0] < max(s1[0], s1[0] + s1[4]):
                        res.append(d1 + abs(s1[0]-s2[0]) + d2 + abs(s1[1]-s2[1]))
                else: # s2[5] == 0
                    # move from s2[0], s2[1] -> s2[0] + s2[4], s2[1]
                    pass # parallel
            d2 += s2[3]
...
assert(find_intersections(parse_line("R8,U5,L5,D3"), parse_line("U7,R6,D4,L4")) == [30, 40])

def find_closest_distance(l1, l2):
    return min(find_intersections(l1, l2))

assert(find_closest_distance(parse_line("R8,U5,L5,D3"), parse_line("U7,R6,D4,L4")) == 30)
assert(find_closest_distance(parse_line("R75,D30,R83,U83,L12,D49,R71,U7,L72"), parse_line("U62,R66,U55,R34,D71,R55,D58,R83")) == 610)
assert(find_closest_distance(parse_line("R98,U47,R26,D63,R33,U87,L62,D20,R33,U53,R51"), parse_line("U98,R91,D20,R16,D67,R40,U7,R15,U6,R7")) == 410)
