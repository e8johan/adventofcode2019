...
intersections = []
for x in range(width_of(world)):
    for y in range(height_of(world)):
        if look_at(x, y, world) == "#":
            if look_at(x-1, y, world) == "#" and \
               look_at(x+1, y, world) == "#" and \
               look_at(x, y-1, world) == "#" and \
               look_at(x, y+1, world) == "#":
...
def to_ascii(main, pa, pb, pc, live):
    res = []
    res += to_ascii_line(main)
    res += to_ascii_line(pa)
    res += to_ascii_line(pb)
    res += to_ascii_line(pc)
    res += to_ascii_line(live)
    return res

memory[0] = 2
cpu.reset(memory, to_ascii("ABABCCBABC","L48L6L8R6", "L8L8R4R6R6", "L48R6L8", "n"))
res_b = -1
line = ""
while not cpu.is_halted():
    while not cpu.has_output() and not cpu.is_halted():
        cpu.step()
    while cpu.has_output():
        o = cpu.output()
        if o == 10:
